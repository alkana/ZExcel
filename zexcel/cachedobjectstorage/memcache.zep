...
class Memcache extends CacheBase implements ICache
{
        ...
        if (this->currentCellIsDirty && !empty(this->currentObjectID)) {
            ...
            if (!this->memcache->replace(this->cachePrefix . this->currentObjectID . ".cache", obj, null, this->cacheTime)) {
                if (!this->memcache->add(this->cachePrefix . this->currentObjectID . ".cache", obj, null, this->cacheTime)) {
                    this->__destruct();
                    throw new \ZExcel\Exception("Failed to store cell {this->currentObjectID} in MemCache");
                }
            }
    ...
    public function isDataSet(pCoord)
    {
        ...
        if (parent::isDataSet(pCoord)) {
            ...
            let success = this->memcache->get(this->cachePrefix.pCoord.".cache");
            if (success === false) {
                //    Entry no longer exists in Memcache, so clear it from the cache array
                parent::deleteCacheData(pCoord);
                throw new \ZExcel\Exception("Cell entry ".pCoord." no longer exists in MemCache");
            }
            return true;
        }
        return false;
    ...
    public function getCacheData(pCoord)
    {
        ...
        if (parent::isDataSet(pCoord)) {
            let obj = this->memcache->get(this->cachePrefix . pCoord . ".cache");
            if (obj === false) {
                //    Entry no longer exists in Memcache, so clear it from the cache array
                parent::deleteCacheData(pCoord);
                throw new \ZExcel\Exception("Cell entry {pCoord} no longer exists in MemCache");
    ...
    public function copyCellCollection(<\ZExcel\Worksheet> parent)
    {
        ...
        for cellID in cacheList {
            if (cellID != this->currentObjectID) {
                let obj = this->memcache->get(this->cachePrefix.cellID.".cache");
                if (obj === false) {
                    //    Entry no longer exists in Memcache, so clear it from the cache array
                    parent::deleteCacheData(cellID);
                    throw new \ZExcel\Exception("Cell entry {cellID} no longer exists in MemCache");
                }
                if (!this->memcache->add(newCachePrefix . cellID . ".cache", obj, null, this->cacheTime)) {
                    this->__destruct();
                    throw new \ZExcel\Exception("Failed to store cell {cellID} in MemCache");
                }
            }
        }
    ...
    {
        var memcacheServer, memcachePort, cacheTime, baseUnique;
        
        let memcacheServer = (isset(arguments["memcacheServer"])) ? arguments["memcacheServer"] : "localhost";
        let memcachePort = (isset(arguments["memcachePort"])) ? arguments["memcachePort"] : 11211;
        ...
        if (is_null(this->cachePrefix)) {
            ...
            if (!this->memcache->addServer(memcacheServer, memcachePort, false, 50, 5, 5, true, [this, "failureCallback"])) {
                throw new \ZExcel\Exception("Could not connect to MemCache server at {memcacheServer}:{memcachePort}");
            }
    ...
    {
        throw new \ZExcel\Exception("memcache {host}:{port} failed");
    }
    ...
    {
        var cacheList = this->getCellList(), cellID;
