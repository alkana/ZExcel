...
class PHPTemp extends CacheBase implements ICache
{
    ...
    public function copyCellCollection(<\ZExcel\Worksheet> parent)
    {
        ...
        let newFileHandle = fopen("php://temp/maxmemory:" . this->memoryCacheSize, "a+");
        //    Copy the existing cell cache data to the new stream
        fseek(this->fileHandle, 0);
        while (!feof(this->fileHandle)) {
            fwrite(newFileHandle, fread(this->fileHandle, 1024));
        }
        let this->fileHandle = newFileHandle;
    }
    ...
    public function __construct(<\ZExcel\Worksheet> parent, array arguments = [])
    {
        ...
        if (is_null(this->fileHandle)) {
            let this->fileHandle = fopen("php://temp/maxmemory:" . this->memoryCacheSize, "a+");
        }
    }
