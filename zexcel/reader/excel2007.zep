...
class Excel2007 extends Abstrac implements IReader
{
	...
	public function load(string pFilename)
	{
		var excel, zipClass, zip, wbRels, rel, relationships,
			...
			charts, chartDetails,
			xmlCore, xmlWorkbook, sheets, eleSheet, docSheet, docProps;
			...
			switch (rel["Type"]) {
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					if isset(xmlWorkbook->sheets) {
						let sheets = reset(xmlWorkbook->sheets);
						...
						for eleSheet in sheets {
							...
							if (this->_loadSheetsOnly == true && !in_array((string) eleSheet["name"], this->_loadSheetsOnly)) {
							...
							// @TODO add code from lines 653 - 1553

							excel->addSheet(docSheet, eleSheet["sheetId"]);
						}
					}
