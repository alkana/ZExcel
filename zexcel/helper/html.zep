...
class Html
{
    ...
    protected function initialise()
    {
        ...
        let this->bold = false;
        let this->italic = false;
        let this->underline = false;
        let this->superscript = false;
        let this->subscript = false;
        let this->strikethrough = false;
    ...
    public function toRichTextObject(var html)
    {
        var dom, loaded;
        ...
        let loaded = dom->loadHTML(html);

        //    Discard excess white space
        let dom->preserveWhiteSpace = false;

        let this->richTextObject = new \ZExcel\RichText();
    ...
    {
        var text, richtextRun;
        
        let text = this->stringData;
        
        if (trim(text) === "") {
            return;
        }

        let richtextRun = this->richTextObject->createTextRun(this->stringData);
        ...
        if (this->color) {
            richtextRun->getFont()->setColor( new \ZExcel\Style\Color( "ff" . this->color ) );
        }
        ...
        if (this->underline) {
            richtextRun->getFont()->setUnderline(\ZExcel\Style\Font::UNDERLINE_SINGLE);
        }
    ...
    {
        return trim(self::colourMap[rgb]);
    }
    ...
    protected function startFontTag(tag)
    {
        ...
        for attribute in tag->attributes {
            let attributeName = strtolower(attribute->name);
    ...
    {
        let this->stringData = this->stringData . PHP_EOL;
    }
    ...
    protected function parseTextNode(<\DOMText> textNode)
    {
        ...
        let domText = preg_replace("/\s+/u", " ", ltrim(textNode->nodeValue));
        let this->stringData = this->stringData . domText;
    ...
    protected function handleCallback(element, callbackTag, callbacks)
    {
        ...
        if (isset(callbacks[callbackTag])) {
            let elementHandler = callbacks[callbackTag];
    ...
    protected function parseElementNode(<\DOMElement> element)
    {
        ...
        let callbackTag = strtolower(element->nodeName);
        let this->stack[] = callbackTag;

        this->handleCallback(element, callbackTag, this->startTagCallbacks);

        this->parseElements(element);
        
        let this->stringData = this->stringData . " ";
